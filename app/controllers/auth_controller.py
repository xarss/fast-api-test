...
def login_google_user_endpoint(
    dto: GoogleLoginDto, db: Session = Depends(get_db)
) -> AccessTokenResponse:
...
def login_user_endpoint(
    dto: LoginDto, db: Session = Depends(get_db)
) -> AccessTokenResponse:
...
def register_user_endpoint(
    dto: RegisterDto, db: Session = Depends(get_db)
) -> AccessTokenResponse:
    return register_user(
        db=db, email=dto.email, username=dto.username, password=dto.password
    )
