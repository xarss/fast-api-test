...
from app.services.user_service import create_user, get_user_by_id, get_users, update_user, update_user_role
from app.models.user_models import CreateUserDto, UpdateUserRoleDto, UserResponseDto, UpdateUserDto
...
def create_user_endpoint(dto: CreateUserDto, db: Session = Depends(get_db)):
    return create_user(db=db, username=dto.username, email=dto.email, role_id=dto.role_id)
...
def update_user_role_endpoint(id: int, dto: UpdateUserRoleDto, db: Session = Depends(get_db)):
