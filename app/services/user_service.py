...
def create_user(
    db: Session, username: str, email: str, role_id: int = 1
) -> UserResponseDto:
    ...
    db_user = User(username=username, email=email, role_id=role_id)
...
def get_user_by_id(db: Session, id: int) -> User:
    ...
    if not db_user:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"Could not find user with id '{id}'"
        )
...
def get_user_by_email(db: Session, email: str) -> User:
    ...
    if not db_user:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"Could not find user with email '{email}'"
        )
